...
BEGIN
	...
	DECLARE @FullFilePath varchar(max);
	DECLARE @DT DateTime;
	...
	DECLARE @ADWSchemaName Varchar(1000)
	DECLARE @ADBDestinationTableName Varchar(1000);
	...
	SET @Blobpath = LTRIM(RTRIM(@Blobpath));
	SET @FullFilePath = @Blobpath + '' + @FileName;
	SET @DT = GETUTCDATE();
	SET @index1 = CHARINDEX('/', @Blobpath);
	...
	SET @ContainerName = SUBSTRING(@Blobpath,1,@index1-1)
	PRINT 'Container Name ' + @ContainerName
	SET @index2 = CHARINDEX('/', @Blobpath,@index1+1);	
	SET @index3 = CHARINDEX('/', @Blobpath,@index2+1);
	SET @index6 = CHARINDEX('/', @Blobpath,@index3+1);
    SET @index4 = CHARINDEX('~', @Blobpath,@index3+1);	
	SET @index5 = CHARINDEX('~', @Blobpath,@index4+1);	
	IF (@index2 = 0 or @index3 = 0 or @index4 = 0 or @index5 = 0)
	BEGIN
			SET @ErMsg = 'Could not find Seperator';
			Raiserror(@ErMsg,16,1);
	END

	SET @ProductName = SUBSTRING(@Blobpath, @index1+1,@index2-(@index1+1));
	PRINT 'Product Name ' + @ProductName
	 
	SET @EntityName = SUBSTRING(@Blobpath, @index3+1,@index6-(@index3+1));
	PRINT 'Search Entity ' + @EntityName
	SET @batchid = SUBSTRING(@Blobpath, @index4+1,@index5-(@index4+1));
	...
	SELECT 
		  ...
		  FROM [dev_test_db].[adfmetadata_tbl]
		   Where UPPER(LTRIM(RTRIM([EntityName]))) = UPPER(LTRIM(RTRIM(@EntityName)))
	 
	IF( @ID IS NULL )
	BEGIN
		   	SET @ErMsg = 'This File is not Setup for Ingestion ' + @FullFilePath;
			Raiserror(@ErMsg,16,1);
	END
	ELSE
	BEGIN
	...
	EXEC [dev_test_db].[insert_FilesToProcess]	
												@ADWID = @NewID,
												@batchid = @batchid,
												@Blobpath = @Blobpath,
												@FileName = @FileName,
		...
		Select @FilesToProcessID=ID From [dev_test_db].Batch_Execution_Detail_Statistic Where Consumption_Data_Warehouse_Id=@NewID;
		
			IF @@ROWCOUNT = 0
			BEGIN
					
					SET @ErMsg = 'Error Retrieving Record from Customer_Service_Analytics.Batch_Execution_Detail_Statistic Table for Consumption_Data_Warehouse_Id  ' + CAST(@NewID AS Varchar(36));
						PRINT @ErMsg;
					Raiserror(@ErMsg,16,1);
			END
			ELSE
